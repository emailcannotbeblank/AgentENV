...
impl ThreadInfo {
    ...
    fn verify_identity(&self, pid: i32) -> Result<()> {
        ...
        if current.starttime != self.starttime {
            bail!(
                "tid={} identity changed while binding (start time {} -> {})",
                self.tid,
                self.starttime,
                current.starttime
            );
...
fn bind_threads(pid: i32, targets: &[&ThreadInfo], desired: &CpuSet, cores: &str) -> Result<()> {
    let originals: Vec<(&ThreadInfo, CpuSet)> = targets
        .iter()
        .map(|&thread| {
            thread.verify_identity(pid)?;
            let original = sched_getaffinity(Pid::from_raw(thread.tid)).with_context(|| {
                format!(
                    "failed to read original affinity of tid={} ({:?})",
    ...
    for (index, (thread, _)) in originals.iter().enumerate() {
        ...
        let actual = match sched_getaffinity(Pid::from_raw(thread.tid)) {
            Ok(actual) => actual,
            Err(error) => {
                return Err(error_after_rollback(
                    pid,
        ...
        if let Err(error) = thread.verify_identity(pid) {
            return Err(error_after_rollback(
                pid,
                format!("thread identity changed after binding: {error:#}"),
                &originals[..applied],
            ));
        }
        if actual != *desired {
...
fn error_after_rollback(
    ...
    for (thread, original) in applied.iter().rev() {
        ...
        match sched_setaffinity(Pid::from_raw(thread.tid), original) {
            ...
            Ok(()) => match sched_getaffinity(Pid::from_raw(thread.tid)) {
                Ok(actual) => {
                    if let Err(error) = thread.verify_identity(pid) {
                        failures.push(format!(
                            "tid={} (identity changed after restore: {error:#})",
                            thread.tid
                        ));
